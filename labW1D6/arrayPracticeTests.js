...
describe("filter practice", function () {
    ...
    it("includes even", function () {
        assert.strictEqual(includesEvenNum(numArray), 0);
        assert.strictEqual(includesEvenNum([1,3,5]), undefined);
    });

    /* complete the following similar to includes even test */
    it("includes even age" , function () {
        assert.strictEqual(includesEvenAge(numArray), 300);
        assert.strictEqual(includesEvenAge([1,3,5]), undefined);
    });
});